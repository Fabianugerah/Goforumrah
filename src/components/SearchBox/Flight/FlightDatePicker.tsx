...
interface FlightDatePickerDropdownProps {
  flightType: 'round-trip' | 'one-way';
  departureDate: Date | null;
  returnDate: Date | null;
  datePickerType: 'departure' | 'return';
  onSelectDeparture: (date: Date) => void;
  onSelectReturn: (date: Date | null) => void;
}

export default function FlightDatePickerDropdown({
  flightType,
  departureDate,
  returnDate,
  datePickerType,
  ...
  const handleDateClick = (date: Date) => {
    if (flightType === 'one-way') {
      ...
      // Round-trip logic
      if (!departureDate || datePickerType === 'departure') {
        onSelectDeparture(date);
      } else {
  ...
  const isDateInRange = (date: Date) => {
    ...
    if (!returnDate && hoverDate && datePickerType === 'return') {
      const start = departureDate < hoverDate ? departureDate : hoverDate;
      const end = departureDate < hoverDate ? hoverDate : departureDate;
  ...
  const isDateDisabled = (date: Date) => {
    ...
    // For return date, disable dates before departure
    if (flightType === 'round-trip' && datePickerType === 'return' && departureDate) {
        ...
        <div className="datepicker-month">
          ...
          <div className="datepicker-days">
            {getDaysInMonth(currentMonth).map((date, index) => {
              ...
              const isDeparture = departureDate && date.toDateString() === departureDate.toDateString();
              ...
              return (
                <button
                  ...
                  className={`datepicker-day 
                    ${isDeparture ? 'datepicker-day--selected-start' : ''}
                    ${isReturn ? 'datepicker-day--selected-end' : ''}
                    ${isInRange ? 'datepicker-day--in-range' : ''} 
                    ${isDisabled ? 'datepicker-day--disabled' : ''}`
                  }
                  onMouseEnter={() => {
                    if (flightType === 'round-trip' && datePickerType === 'return' && departureDate && !returnDate) {
                      setHoverDate(date);
                    }
                  }}
